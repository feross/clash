...
string Job::SwitchParsingTarget(char matched, string& job_str_copy) {
    switch(matched) {
        case '`': {
...
string Job::ParseDoubleQuote(string& job_str_copy) {
...
string Job::ParseSingleQuote(string& job_str_copy) {
...
string Job::ParseBackslash(string& job_str_copy, char mode) {
...
string Job::ParseVariable(string& job_str_copy) { //TODO: push at front & reparse (may introduce words)
...
string Job::ParseBacktick(string& job_str_copy) { //TODO: push at front & reparse (may introduce words)
    string quoted = string();
    int match_index;
    while((match_index = strcspn(job_str_copy.c_str(), "`\\")) != job_str_copy.size()) {
        char matched = job_str_copy[match_index];
...
string Job::ParseTilde(string& job_str_copy) { //TODO: push at front & reparse (may introduce words)
